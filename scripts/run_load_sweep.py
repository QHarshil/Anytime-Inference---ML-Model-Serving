...
Usage:
    ...
    python scripts/run_load_sweep.py --replot
"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--replot",
        action="store_true",
        help="Redraw the figure from an existing --output CSV instead of measuring. "
        "Needs no models and no ONNX Runtime pool.",
    )
    ...
    if args.replot:
        # Everything the figure annotates comes from the serving config, which is
        # committed, so a checkout can redraw this figure from the committed CSV without
        # a model directory, an ONNX Runtime pool, or a measurement it has no quiet host
        # for. The capacity in the title is derived the same way it was when the CSV was
        # written, so a replot cannot silently retitle the picture.
        import pandas as pd

        if not args.output.is_file():
            raise SystemExit(
                f"{args.output} is missing, so there is nothing to replot. Measure first:\n"
                f"    python scripts/run_load_sweep.py"
            )
        frame = pd.read_csv(args.output)
        LOGGER.info("Read %d rows from %s", len(frame), args.output)
        _draw_figure(
            frame,
            args.figure,
...
def _draw_figure(
    ...
    *,
    ...
    """Draw the sweep figure from a frame, whether it was just measured or just read.

    Split out of `_write_outputs` so `--replot` can reach it. Every other figure in this
    repository is drawn by a script that measures nothing, and `benchmarks.md` says that
    separation is the point: a run drifts by up to 10% back to back, so a figure
    regenerated by re-measuring disagrees with the tables beside it for reasons that have
    nothing to do with the code. This sweep was the one that could not, which also meant
    it was the one figure a reader could not reproduce from a checkout.
    """
    import matplotlib
    ...
    styles = {
        "accurate-only": {"color": "#b2432f", "marker": "o", "label": f"{accurate_name} only"},
