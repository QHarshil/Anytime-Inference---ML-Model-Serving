...
python scripts/run_load_sweep.py --replot     # redraw that figure from the CSV, measuring nothing
...
The load sweep was the one exception, measuring and drawing in a single pass, so its
figure could not be redrawn without running the models again. `--replot` closes that: it
